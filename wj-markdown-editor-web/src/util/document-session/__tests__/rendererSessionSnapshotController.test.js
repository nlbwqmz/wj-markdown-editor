...
test('renderer 公共 snapshot controller 在 replay-store 恢复时，也必须走 recent-missing 与 closePrompt 的统一副作用链', () => {
  assert.ok(rendererSessionSnapshotControllerModule, '缺少 renderer 公共 snapshot controller')
  ...
  assert.equal(typeof controller.replaySnapshot, 'function')

  const replayed = controller.replaySnapshot({
    isRecentMissing: true,
    recentMissingPath: 'C:/docs/missing.md',
    content: '# body',
    closePrompt: {
