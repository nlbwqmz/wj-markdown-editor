...
- 如果任务级代码评审发现问题，必须在当前任务内自动修复并重新评审，直到无问题后才能继续
- 设计中识别出的风险点必须映射到实施任务中，在任务内完成消减、验证和收口
- 不允许把已识别风险仅留给最后的手工验证兜底
- 全部任务执行完成后，必须针对本次更新范围执行一次全量代码评审
- 最终放行条件必须是：无评审问题、无功能前后不一致、无功能降级
...
- 设计中的每个风险点都必须在实施计划中落成明确任务或任务子目标
- 每个风险点都必须对应明确的自动化验证、代码评审检查点或实现约束
- 手工回归只作为 Electron 真环境补充验证，不能替代风险本身的实现闭环
- 如果某个风险点在当前任务中无法关闭，必须阻塞后续任务推进，而不是留到最后统一处理
...
- 每个任务的代码评审问题都已在对应任务内修复并复评通过
- 已完成一次针对本次更新范围的全量代码评审，且评审结论无问题
- 已确认不存在功能前后不一致和功能降级
...
- 先补基线测试
- 将每个风险点前置纳入任务拆分，并在任务内闭环
- 逐任务推进并在任务间做代码评审；如发现问题，必须在当前任务内自动修复并复评
- 每删除一类兼容入口都补充反向断言
- 全部任务结束后执行一次本次更新范围的全量代码评审，并以“无问题、无功能不一致、无功能降级”为放行条件
