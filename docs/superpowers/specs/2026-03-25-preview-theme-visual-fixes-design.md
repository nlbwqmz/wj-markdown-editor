...
代码块外壳背景不新增独立的 dark 专用变量族，而是明确复用现有代码块变量：

- `--wj-preview-code-block-background-color`
- `--wj-preview-code-block-text-color`
- `--wj-preview-code-toolbar-background-color`

本轮若代码块外壳在暗黑模式下需要修正背景或工具栏表面，必须通过这组既有变量完成，不接受元素级补丁兜底。
...
复制按钮的 `title` / `aria-label` 文案应保持单一来源。优先复用现有“复制”语义；若当前 markdown-it 输出链路暂时无法直接读取 i18n，则至少应收敛为单一常量，而不是在 HTML 结构中散落多份硬编码。
...
- 矩阵中的受影响主题显式覆盖 `kbd` 变量，且主题文件中不再保留 `kbd` 选择器规则
- `juejin`、`smart-blue`、`vuepress`、`mk-cute`、`scrolls`、`markdown-here` 的 dark 分支分别满足“7.5.7 主题 dark 最小变量覆盖矩阵”的必覆盖 token 集
- 若某主题需要修正代码块外壳表面，必须通过 `--wj-preview-code-block-background-color`、`--wj-preview-code-block-text-color`、`--wj-preview-code-toolbar-background-color` 这组变量完成
