...
- 支持图片粘贴上传、拖拽上传、截图上传
- 支持文件、视频、音频插入（含本地资源）
- 编辑页预览区支持本地图片、视频、音频、文件链接右键菜单，可直接在资源管理器中打开，或删除本地资源并精确同步清理当前 Markdown 片段
...
- 编辑区与预览区精准同步滚动
- 自动提取文档目录并导航
- 支持标题锚点与文档内 Hash 链接跳转，预览区点击后可平滑滚动到目标位置
- 图片预览支持缩放、翻页、旋转
- 支持全局主题切换（明亮 / 暗黑）
- 支持多种种代码高亮主题和预览主题
...
支持为 Markdown 标题自动生成锚点，并在预览区点击文档内 `Hash` 链接后平滑滚动到对应位置。
...
上面这种“标题本身不含空格”的中文场景可以直接使用。渲染后链接会自动编码，并跳转到当前文档对应标题。

如果标题包含英文、空格或符号，建议直接按生成后的锚点写链接：

```md
## Hello World
## API / SDK

跳转到[Hello World](#hello-world)
跳转到[API / SDK](#api-%2F-sdk)
```

### 标题链接生成规则

- 仅 `#` 到 `######` 标题会自动生成锚点
- 锚点基于标题中的纯文本和行内代码文本生成
- 生成时会先去掉首尾空白，再统一转为小写
- 标题中的连续空白会折叠为一个 `-`
- 结果会再做 URL 编码，因此中文和特殊符号在最终 `id` 中会变成编码结果
- 如果存在同名标题，后续同名标题会自动追加 `-1`、`-2` 等后缀

示例：

| 标题 | 推荐链接写法 | 实际生成的 `id` |
| --- | --- | --- |
| `## 快速开始` | `[跳转](#快速开始)` | `%E5%BF%AB%E9%80%9F%E5%BC%80%E5%A7%8B` |
| `## Hello World` | `[跳转](#hello-world)` | `hello-world` |
| 第二个 `## Hello World` | `[跳转](#hello-world-1)` | `hello-world-1` |
| `## API / SDK` | `[跳转](#api-%2F-sdk)` | `api-%2F-sdk` |

如果你需要手动拼接链接，按上面的规则推导 `id` 即可；如果是中文且标题本身不带空格，直接写成 `[文本](#标题)` 通常也能正常使用。
